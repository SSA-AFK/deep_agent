...
- 移除 RAGFlow 后的 V3 当前 24 条真实评测完成率为 58.3%（14/24），公开搜索路由稳定性仍需优化；随后增加每角色一次调度与 45 秒透明超时降级，同一中文公开研究任务在 48.6 秒成功返回真实 URL。该次验证不改写 V3 原始基线。
...
5. `.env`、`(1).env` 和 `deepseek_api.env` 含敏感配置；不得提交、打印或复制其值。
