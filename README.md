...
- 后端：68 个自动化测试通过，包括模型兼容、URL 引用保留、路径边界、上传限制、只读 SQL、服务降级、任务状态、事件恢复、真实评测执行边界和离线端到端链路。
...
模型和外部服务通过环境变量配置。只记录变量名，不要把值写入日志或提交：DeepSeek 使用 `DEEPSEEK_API_KEY`（可选 `DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL`，默认 `deepseek-v4-flash`）；也可使用 `LLM_MODEL`（显式优先）或 Qwen 的 `LLM_QWEN3`、`LLM_QWEN_MAX` 与 OpenAI 兼容变量。
