...
test("research flow reaches a sourced report", async ({ page }) => {
  ...
  await page.screenshot({ path: "../docs/interview/assets/task-input.png", fullPage: true });
  ...
  await page.screenshot({ path: "../docs/interview/assets/plan-confirmation.png", fullPage: true });
  ...
  await page.screenshot({ path: "../docs/interview/assets/active-agent-run.png", fullPage: true });
  await page.getByText("服务状态").click();
  await expect(page.getByText("已降级为 demo").first()).toBeVisible();
  await page.screenshot({ path: "../docs/interview/assets/transparent-degradation.png", fullPage: true });
  await page.getByText("服务状态").click();
