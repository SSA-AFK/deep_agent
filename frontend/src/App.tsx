...
export default function App() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (query.trim()) { setSubmitted(query.trim()); setConfirmed(false); setCompleted(false); }
  };
  return <main className="workbench">
    ...
    <section className="conversation" aria-labelledby="page-title">
      ...
      {submitted ? <><article className="message user-message"><span>你的研究问题</span><p>{submitted}</p></article>{completed ? <ReportMessage /> : confirmed ? <AgentRunMessage onComplete={() => setCompleted(true)} /> : <PlanMessage onConfirm={() => setConfirmed(true)} onCancel={() => setSubmitted(null)} />}</> : <section className="empty-state"><FileText size={28} /><h2>开始一项复杂研究</h2><p>说明目标、限制条件和你希望得到的输出。</p><div className="starters">{STARTERS.map((starter) => <button key={starter} type="button" onClick={() => setQuery(starter)}>{starter}</button>)}</div></section>}
