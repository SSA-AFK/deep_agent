...
export default function App() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setThreadId(nextThreadId); setSubmitted(query.trim()); setConfirmed(false); setCompleted(false); setError(null);
    } catch { setError("任务创建失败，请检查服务状态后重试。"); }
  };
  const confirm = async () => {
    if (!threadId) return;
    try { await api.confirm(threadId); setConfirmed(true); setError(null); }
