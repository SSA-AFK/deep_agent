...
export function ReportMessage({ result, threadId }: { result?: string | null; threadId: string }) {
  ...
  return <article className="report-message" aria-label="研究报告">
    ...
    <footer><span>这份结果有帮助吗？</span><button type="button" aria-label="有帮助" onClick={() => sendFeedback(true)}><ThumbsUp size={16} /></button><button type="button" aria-label="需要改进" onClick={() => sendFeedback(false)}><ThumbsDown size={16} /></button><button type="button" className="export" onClick={exportPdf}><Download size={16} /> 导出 PDF</button></footer>
