...
export function AgentRunMessage({ onComplete }: { onComplete(): void }) {
  return <article className="agent-run" aria-label="多 Agent 执行进度">
    ...
    <div className="agent-rows">{agents.map(({ name, action, mode, icon: Icon }, index) => <div className="agent-row" key={name}><Icon size={17} /><div><strong>{name}</strong><span>{action}</span></div><span className={`mode ${mode}`}>{mode === "live" ? "实时来源" : "演示来源"}</span>{index === 1 ? <ShieldAlert size={16} className="degrade-icon" aria-label="已降级" /> : <Check size={16} className="check-icon" aria-label="状态正常" />}</div>)}</div>
